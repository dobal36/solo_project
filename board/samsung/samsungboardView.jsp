...
<script>
	function samsungdelCheck(){
		if(confirm("삭제하시겠습니까?")){
			location.href = "/myapp/board/samsung/samsungboardDel?no=${vo.no}";
		}
	}
	$(function(){
		function replyListAll(){
			var url = "/myapp/reply/samsunglist";
			var params = "no=${vo.no}";
			$.ajax({
				url:url,
				data:params,
		...
		$("#replyFrm").submit(function(){
			...
			if($("#coment").val()==""){
				...
				$.ajax({
					url:'/myapp/reply/samsungwriteOk',
					data:params,
					type:'POST',
					success:function(r){
						$("#coment").val("");
        ...
        $(document).on('submit','#replyList form',function(){
            ...
            let url2 = '/myapp/reply/samsungeditOk';
            $.ajax({
                url : url2,
                data : params2,
                type : 'POST',
...
<div class="boardcontainer">
	...
	<c:if test="${logStatus=='Y'}">
		<form method='post' id="replyFrm">
			...
			<textarea name="coment" id='coment' style="width:500px; height:50px; margin-bottom:30px;"></textarea>
			<input type="submit" value="댓글등록" style="position:absolute; width:100px; height:56px;"/>
		</form>
	</c:if>
	<div id="edit">
		<c:if test="${logNick==vo.nickname}">
			<a href="/myapp/board/samsung/samsungboardEdit?no=${vo.no}">수정</a>
			<a href="javascript:samsungdelCheck()">삭제</a>
		</c:if>
	</div>
	<div id="replyList">
